...
$JEMAT_Allotment_Card = $_FILES['MAT_A_Card'];
$MAT_Rank_Card = $_FILES['MAT_R_Card'];
...
$JEMAT_Allotment_Card_Name = $JEMAT_Allotment_Card['name'];
$JEMAT_Allotment_Card_Path = $JEMAT_Allotment_Card['tmp_name'];
$JEMAT_Allotment_Card_Error = $JEMAT_Allotment_Card['error'];
$JEMAT_Allotment_Card_Card_Size = $JEMAT_Allotment_Card['size'];
if($JEMAT_Allotment_Card_Error == 0 && $JEMAT_Allotment_Card_Size < 204000){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_JEMAT_Allotment_Card.jpg';
	move_uploaded_file($JEMAT_Allotment_Card_Path, $Destination_File);
	$JEMAT_Allotment_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$MAT_Rank_Card_Name = $MAT_Rank_Card['name'];
$MAT_Rank_Card_Path = $MAT_Rank_Card['tmp_name'];
$MAT_Rank_Card_Error = $MAT_Rank_Card['error'];
$MAT_Rank_Card_Size = $MAT_Rank_Card['size'];
if($MAT_Rank_Card_Error == 0 && $MAT_Rank_Card_Size < 204000 ){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_MAT_Rank_Card.jpg';
	move_uploaded_file($MAT_Rank_Card_Path, $Destination_File);
	$MAT_Rank_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$sql = "INSERT INTO `$table_name` (Student_Id,Student_Name,	Course_Under_By,	Entrance_Type,	Course_Name,	JEMAT_Roll,	RANK,	Course_Type,	CATEGORY,	AADHAAR,	Passing_Year,	RegN,	Fathers_Name,	Mothers_Name,	DOB,	Nationality,	Gender,	Blood_Group,	Religion,	Gurdian_Name,	Gurdian_Address,	Gurdian_Contact,	Relation_With_Student,	Residential_Status,	Student_Address,	Student_State,	Student_District,	Student_Pin,	Student_Mobile,	Student_Altno,	Student_Email,	Examination_Name,	Board,
Paper_1_Marks,Paper_2_Marks, Paper_3_Marks, Paper_4_Marks, Paper_5_Marks, Paper_6_Marks, Paper_7_Marks, Paper_8_Marks,	Division,	CGPA,	Marks_Sheet,	Aadhaar_Card,	Photo,	Signature,	DOB_Proof,	Anti_Ragging_Declaration,	Domicile_Certificate,	Caste_Certificate,	JEMAT_Allotment_Card,	MAT_Rank_Card	)
VALUES ('$S_Id','$S_Name',	'$Course_Under_By',	'$Entrance_Type',	'$Course',	'$JEMAT_Roll',	'$RANK',	'$Course_Type',	'$CATEGORY',	'AADHAAR',	'$Passing_Year',	'$RegN',	'$Fathers_Name',	'$Mothers_Name',	'$DOB',	'$Nationality',	'$Gender',	'$Blood_Group',	'$Religion',	'$Gurdian_Name',	'$Gurdian_Address',	'$Gurdian_Contact',	'$Relation_With_Student',	'$Residential_Status',	'$Student_Address',	'$Student_State',	'$Student_District',	'$Student_Pin',	'$Student_Mobile',	'$Student_Altno',	'$Student_Email',	'$Examination_Name',	'$Board','$Paper_1_Marks','$Paper_2_Marks','$Paper_3_Marks','$Paper_4_Marks',	'$Paper_5_Marks',	'$Paper_6_Marks',	'$Paper_7_Marks', '$Paper_8_Marks','$Division',	'$CGPA',	'$Marks_Sheet',	'$Aadhaar_Card',	'$Photo',	'$Signature',	'$DOB_Proof',
	'$Anti_Ragging_Declaration',	'$Domicile_Certificate',	'$Caste_Certificate',	'$JEMAT_Allotment_Card',	'$MAT_Rank_Card')";
