...
$JECA_Allotment_Card = $_FILES['JECA_A_Card'];
$JECA_Rank_Card = $_FILES['JECA_R_Card'];
...
$JECA_Allotment_Card_Name = $JECA_Allotment_Card['name'];
$JECA_Allotment_Card_Path = $JECA_Allotment_Card['tmp_name'];
$JECA_Allotment_Card_Error = $JECA_Allotment_Card['error'];
$JECA_Allotment_Card_Card_Size = $JECA_Allotment_Card['size'];
if($JECA_Allotment_Card_Error == 0 && $JECA_Allotment_Card_Size < 204000){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_JECA_Allotment_Card.jpg';
	move_uploaded_file($JECA_Allotment_Card_Path, $Destination_File);
	$JECA_Allotment_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$JECA_Rank_Card_Name = $JECA_Rank_Card['name'];
$JECA_Rank_Card_Path = $JECA_Rank_Card['tmp_name'];
$JECA_Rank_Card_Error = $JECA_Rank_Card['error'];
$JECA_Rank_Card_Size = $JECA_Rank_Card['size'];
if($JECA_Rank_Card_Error == 0 && $JECA_Rank_Card_Size < 204000 ){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_JECA_Rank_Card.jpg';
	move_uploaded_file($JECA_Rank_Card_Path, $Destination_File);
	$JECA_Rank_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$sql = "INSERT INTO `$table_name` (Student_Id,Student_Name,	Course_Under_By,	Entrance_Type,	Course_Name,	JECA_Roll,	RANK,	Course_Type,	CATEGORY,	AADHAAR,	Passing_Year,	RegN,	Fathers_Name,	Mothers_Name,	DOB,	Nationality,	Gender,	Blood_Group,	Religion,	Gurdian_Name,	Gurdian_Address,	Gurdian_Contact,	Relation_With_Student,	Residential_Status,	Student_Address,	Student_State,	Student_District,	Student_Pin,	Student_Mobile,	Student_Altno,	Student_Email,	Examination_Name,	Board,
Paper_1_Marks,Paper_2_Marks, Paper_3_Marks, Paper_4_Marks, Paper_5_Marks, Paper_6_Marks, Paper_7_Marks, Paper_8_Marks,	Division,	CGPA,	Marks_Sheet,	Aadhaar_Card,	Photo,	Signature,	DOB_Proof,	Anti_Ragging_Declaration,	Domicile_Certificate,	Caste_Certificate,	JECA_Allotment_Card,	JECA_Rank_Card	)
VALUES ('$S_Id','$S_Name',	'$Course_Under_By',	'$Entrance_Type',	'$Course',	'$JECA_Roll',	'$RANK',	'$Course_Type',	'$CATEGORY',	'AADHAAR',	'$Passing_Year',	'$RegN',	'$Fathers_Name',	'$Mothers_Name',	'$DOB',	'$Nationality',	'$Gender',	'$Blood_Group',	'$Religion',	'$Gurdian_Name',	'$Gurdian_Address',	'$Gurdian_Contact',	'$Relation_With_Student',	'$Residential_Status',	'$Student_Address',	'$Student_State',	'$Student_District',	'$Student_Pin',	'$Student_Mobile',	'$Student_Altno',	'$Student_Email',	'$Examination_Name',	'$Board','$Paper_1_Marks','$Paper_2_Marks','$Paper_3_Marks','$Paper_4_Marks',	'$Paper_5_Marks',	'$Paper_6_Marks',	'$Paper_7_Marks', '$Paper_8_Marks','$Division',	'$CGPA',	'$Marks_Sheet',	'$Aadhaar_Card',	'$Photo',	'$Signature',	'$DOB_Proof',
	'$Anti_Ragging_Declaration',	'$Domicile_Certificate',	'$Caste_Certificate',	'$JECA_Allotment_Card',	'$JECA_Rank_Card')";
