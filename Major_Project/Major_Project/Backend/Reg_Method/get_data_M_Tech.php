...
$PGET_Allotment_Card = $_FILES['PGET_A_Card'];
$GATE_Rank_Card = $_FILES['GATE_R_Card'];
...
$PGET_Allotment_Card_Name = $PGET_Allotment_Card['name'];
$PGET_Allotment_Card_Path = $PGET_Allotment_Card['tmp_name'];
$PGET_Allotment_Card_Error = $PGET_Allotment_Card['error'];
$PGET_Allotment_Card_Card_Size = $PGET_Allotment_Card['size'];
if($PGET_Allotment_Card_Error == 0 && $PGET_Allotment_Card_Size < 204000){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_PGET_Allotment_Card.jpg';
	move_uploaded_file($PGET_Allotment_Card_Path, $Destination_File);
	$PGET_Allotment_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$GATE_Rank_Card_Name = $GATE_Rank_Card['name'];
$GATE_Rank_Card_Path = $GATE_Rank_Card['tmp_name'];
$GATE_Rank_Card_Error = $GATE_Rank_Card['error'];
$GATE_Rank_Card_Size = $GATE_Rank_Card['size'];
if($GATE_Rank_Card_Error == 0 && $GATE_Rank_Card_Size < 204000 ){
	$Destination_File = '../../Soft_Copy/'.$table_name.'/'.$S_Id.'_GATE_Rank_Card.jpg';
	move_uploaded_file($GATE_Rank_Card_Path, $Destination_File);
	$GATE_Rank_Card = $Destination_File;
}
else{
	echo "Error or size >= 200KB";
}

$sql = "INSERT INTO `$table_name` (Student_Id,Student_Name,	Course_Under_By,	Entrance_Type,	Course_Name,	PGET_Roll,	RANK,	Course_Type,	CATEGORY,	AADHAAR,	Passing_Year,	RegN,	Fathers_Name,	Mothers_Name,	DOB,	Nationality,	Gender,	Blood_Group,	Religion,	Gurdian_Name,	Gurdian_Address,	Gurdian_Contact,	Relation_With_Student,	Residential_Status,	Student_Address,	Student_State,	Student_District,	Student_Pin,	Student_Mobile,	Student_Altno,	Student_Email,	Examination_Name,	Board,
Paper_1_Marks,Paper_2_Marks, Paper_3_Marks, Paper_4_Marks, Paper_5_Marks, Paper_6_Marks, Paper_7_Marks, Paper_8_Marks,	Division,	CGPA,	Marks_Sheet,	Aadhaar_Card,	Photo,	Signature,	DOB_Proof,	Anti_Ragging_Declaration,	Domicile_Certificate,	Caste_Certificate,	PGET_Allotment_Card,	GATE_Rank_Card	)
VALUES ('$S_Id','$S_Name',	'$Course_Under_By',	'$Entrance_Type',	'$Course',	'$PGET_Roll',	'$RANK',	'$Course_Type',	'$CATEGORY',	'AADHAAR',	'$Passing_Year',	'$RegN',	'$Fathers_Name',	'$Mothers_Name',	'$DOB',	'$Nationality',	'$Gender',	'$Blood_Group',	'$Religion',	'$Gurdian_Name',	'$Gurdian_Address',	'$Gurdian_Contact',	'$Relation_With_Student',	'$Residential_Status',	'$Student_Address',	'$Student_State',	'$Student_District',	'$Student_Pin',	'$Student_Mobile',	'$Student_Altno',	'$Student_Email',	'$Examination_Name',	'$Board','$Paper_1_Marks','$Paper_2_Marks','$Paper_3_Marks','$Paper_4_Marks',	'$Paper_5_Marks',	'$Paper_6_Marks',	'$Paper_7_Marks', '$Paper_8_Marks','$Division',	'$CGPA',	'$Marks_Sheet',	'$Aadhaar_Card',	'$Photo',	'$Signature',	'$DOB_Proof',
	'$Anti_Ragging_Declaration',	'$Domicile_Certificate',	'$Caste_Certificate',	'$PGET_Allotment_Card',	'$GATE_Rank_Card')";
